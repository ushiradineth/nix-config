        ...
        customCommands = [
          {
            ...
            command = ''
              ...
              MSG=$(echo "$DIFF" | opencode run --agent direct --pure --model "openai/gpt-5.6-luna" "Generate one concise Conventional Commit subject for the provided git diff. Success means the output is only <type>(<scope>): <msg>, uses imperative mood, has no period, stays under 50 characters, and describes the concrete change. If the diff is unclear, choose the safest accurate scope and type without explaining." 2>/dev/null)
              if [ -n "$MSG" ]; then
                git commit -e -m "$MSG"
              fi
            '';
